...
function collectCoverage() {
  ...
  if (!existsSync(coverageFile)) {
    return {
      error:
        "coverage/coverage-final.json não encontrado — execute `npm run test:coverage` primeiro",
      percent: 0,
      covered_lines: 0,
      missing_lines: 0,
      num_statements: 0,
  ...
  let testsFailed = 0;
  let testsTotal = 0;
  const jestResult = run("npx jest --coverage --coverageReporters=json 2>/dev/null");
  try {
    const jj = JSON.parse(jestResult.stdout) as {
      numPassedTests?: number;
      numFailedTests?: number;
      numTotalTests?: number;
    };
    testsPassed = jj.numPassedTests ?? 0;
    testsFailed = jj.numFailedTests ?? 0;
    testsTotal = jj.numTotalTests ?? testsPassed + testsFailed;
  } catch {
    const m = jestResult.stdout.match(/(\d+)\s+passed/);
